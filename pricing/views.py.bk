...
def gcp(request):
    ...
    prd = pr.values('ptype', 'psubtype')
    prd = prd.order_by('ptype', 'psubtype').distinct()
...
def aws_compute_instance(request, offer_code, product_family, instance_type):
    ...
    pr = pr.order_by('location', 'term_type', 'operating_system',
                     'lease_contract_length', 'tenancy', 'purchase_option',
                     'unit')
